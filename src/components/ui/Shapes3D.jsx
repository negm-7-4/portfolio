...
export const WireCube = memo(function WireCube({
  size = 120,
  dur = 30,
  color = "rgba(180,196,220,0.42)",
  className = "",
}) {
  const h = size / 2;
  const faces = [
  ...
  return (
    ...
    >
      <div
        className="spin3d-anim relative"
        style={{
          width: size,
          height: size,
...
export const Gyro = memo(function Gyro({
  size = 150,
  dur = 34,
  color = "rgba(180,196,220,0.4)",
  className = "",
}) {
  const rings = [
      ...
      >
        {rings.map((t, i) => (
          <span
            ...
            style={{ border: `1px solid ${color}`, transform: t }}
          />
        ))}
        <span
          className="absolute left-1/2 top-1/2 h-1.5 w-1.5 -translate-x-1/2 -translate-y-1/2 rounded-full"
          style={{
            background: "rgba(200,212,228,0.8)",
            boxShadow: "0 0 12px rgba(180,196,220,0.8)",
          }}
        />
      </div>
    </div>
...
export const Diamond = memo(function Diamond({
  size = 110,
  dur = 26,

...
function ProofCard({ item, index }) {
  return (
    ...
    >
      <p className="font-display text-3xl font-semibold tracking-tight text-white md:text-4xl">{item.value}</p>
        ...
        <div className="grid items-start gap-12 lg:grid-cols-[0.9fr_1.1fr] lg:gap-16">
          <Reveal className="lg:sticky lg:top-28" dir="left" once>
            ...
            <p className="mt-6 max-w-xl text-base leading-relaxed text-white/58">
              SAMS turns an AI delivery pipeline into a living 3D company. The office is not decoration:
              every desk, meeting, status light and speech bubble reflects real planning, file writes,
              tests, blockers and approval decisions.
            </p>

            <div className="mt-8 grid grid-cols-2 gap-x-4 gap-y-7" aria-label="Verified SAMS project baseline">
              {proof.map((item, index) => <ProofCard key={item.label} item={item} index={index} />)}
            </div>
          ...
          <div className="space-y-10">
            <Reveal once>
              <figure className="overflow-hidden rounded-[1.75rem] border border-white/10 bg-[#10131a] shadow-[0_35px_90px_-30px_rgba(0,0,0,0.85)]">
                <div className="flex items-center justify-between border-b border-white/10 px-4 py-3">
                  ...
                  <span className="text-[9px] uppercase tracking-[0.28em] text-white/45">SAMS · live office state</span>
                ...
                <figcaption className="border-t border-white/8 px-5 py-4 text-xs leading-relaxed text-white/45">
                  One operational surface for agent movement, mission status, security gates, code review and delivery decisions.
                </figcaption>
              </figure>
            </Reveal>
            ...
            <Reveal once>
              <div className="grid gap-4 sm:grid-cols-2">
                <article className="rounded-2xl border border-white/9 bg-white/[0.025] p-6">
                  <p className="text-[10px] font-semibold uppercase tracking-[0.28em] text-white/45">The problem</p>
                  <p className="mt-3 text-sm leading-relaxed text-white/65">
                    Multi-agent demos often stop at conversation. Work overlaps, a model can claim completion without evidence,
                    and the human cannot see where the delivery actually stands.
                  </p>
                </article>
                <article className="rounded-2xl border border-[#9a8ac8]/25 bg-[#9a8ac8]/[0.055] p-6">
                  <p className="text-[10px] font-semibold uppercase tracking-[0.28em] text-[#c7bdf0]">The decision</p>
                  <p className="mt-3 text-sm leading-relaxed text-white/65">
                    Treat every model output as a proposal. Locked claims prevent duplicate work, real exit codes decide whether
                    a build passed, and the CEO keeps the final gate.
                  </p>
                </article>
              </div>
            </Reveal>
            ...
            <Reveal once>
              <div className="rounded-3xl border border-white/10 bg-black/20 p-6 md:p-8">
                <div className="mb-7 flex items-center justify-between gap-4">
                  <div>
                    <p className="text-[10px] font-semibold uppercase tracking-[0.28em] text-white/45">Delivery trace</p>
                    <h4 className="mt-2 font-display text-2xl font-semibold text-white">From intent to evidence</h4>
                  </div>
                ...
                <ol className="relative space-y-2">
                  {deliveryTrace.map((step, index) => (
                    <li key={step.label} className="group grid grid-cols-[2.5rem_1fr] gap-3 rounded-xl p-3 transition-colors hover:bg-white/[0.035]">
                      ...
                      <div>
                        ...
                        <p className="mt-0.5 text-xs leading-relaxed text-white/45">{step.detail}</p>
                      </div>
                    </li>
                  ))}
                </ol>
            ...
            <Reveal once>
              <div className="flex flex-col items-start justify-between gap-5 rounded-2xl border border-white/10 bg-white/[0.03] p-6 sm:flex-row sm:items-center">
                <div>
                  <p className="font-display text-xl font-semibold text-white">Need this depth in your next product?</p>
                  <p className="mt-1 text-sm text-white/45">Let’s turn the complex part into the clearest part.</p>
                </div>
                <a
                  href="#contact"
                  onClick={(event) => {

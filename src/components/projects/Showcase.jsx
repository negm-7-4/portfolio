...
/* ─── Showcase view: sticky scrollytelling with browser-framed previews ─── */
export default function Showcase() {
  ...
  useEffect(() => {
    ...
    const io = new IntersectionObserver(
      (entries) => {
        for (const e of entries) ratios.set(Number(e.target.dataset.idx), e.intersectionRatio);
        let bestIdx = 0, bestRatio = -1;
        for (const [i, r] of ratios) if (r > bestRatio) { bestRatio = r; bestIdx = i; }
        setActive(bestIdx);
      },
      { threshold: Array.from({ length: 21 }, (_, i) => i / 20), rootMargin: "-30% 0px -30% 0px" }
    );
    ...
    <div ref={ref} className="relative">
      ...
      <div className="mb-12 flex items-center justify-center gap-3">
        {projects.map((p, i) => (
          ...
          >
            <motion.span
              animate={{ width: active === i ? 28 : 8, backgroundColor: active === i ? p.color : "rgba(255,255,255,0.2)" }}
      ...
      <div className="grid grid-cols-1 gap-x-10 md:grid-cols-12">
        ...
        <div className="mt-12 flex flex-col gap-24 md:col-span-7 md:col-start-6 md:mt-0 md:gap-40">
          {projects.map((p, i) => {
            const url = (p.github || "").replace(/https?:\/\//, "");
            return (
              <div key={p.title} ref={(el) => (slotRefs.current[i] = el)} data-idx={i} className="relative">
                <BrowserFrame url={url} accent={p.color}>
                  {p.image ? (
                    <img
                      src={p.image} alt={p.title} loading="lazy" decoding="async" fetchpriority="low"
                      width="1200" height="705"
                    ...
                    <div className="absolute inset-0 flex items-center justify-center"
                      ...
                      <div className="text-center">
                        <p className="font-display text-7xl font-bold tracking-tight" style={{ color: `${p.color}55` }}>
                          {p.title.split(" ").map((w) => w[0]).join("")}
                        </p>
                        <p className="mt-3 text-[10px] uppercase tracking-[0.3em] text-white/30">preview · soon</p>
                      </div>
                    </div>
                  )}
                </BrowserFrame>
